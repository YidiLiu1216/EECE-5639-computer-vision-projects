...
SavePath = '.\ResultChair\';
DiffPath ='.\Diff\';
MidPath='.\ResultChair\mid\';
% File suffix
fileExt = '*.jpg';
...
for i=2:len-1
   ...
   diff=diff+double(framesub)/(len-2);
end
%caculate the max average temporal derivate as threshold
threshold=(max(max(diff))-2)/255;
 h=histogram(diff);
% x=0.01:0.01:2.56;
% f=fittype('a1*exp(-((x)/(2^(1/2))*b1)^2)');
% %f=fittype('gauss1');
% t=fit(x',h,f);
% temp_threshold=coeffvalues(t);
%threshold=(max(max(diff))/255;

%generate the mask and put it on original image
 midName = strcat(MidPath,files(2).name);
for i=2:len-1
    ...
    if i==30
        imwrite(uint8(mask)*255,midName);
    end
